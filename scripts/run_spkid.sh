...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db_devel/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           EXEC="gmm_train -v 1 -T 0.001 -N 100 -m 30 -i 1 -n 20 -t 0 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train"
           echo $EXEC && $EXEC || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       # - The name of the world model will be used by gmm_verify in the 'verify' command below.
        EXEC="gmm_train -v 1 -T 0.001 -N 150 -m 30 -i 1 -n 20 -t 0 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train"
        echo $EXEC && $EXEC || exit 1

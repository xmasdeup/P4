	...
	**/
	class CircularIndex
	{
		...
		public:
			...
			CircularIndex(int s=1): index(0), size(s) {
			}
								 ///< constructor defining size based on other CircularIndex
			CircularIndex(CircularIndex &ci): index(ci.index), size(ci.size) {
			}
			void resize(int s) { ///<Set size of circular index
				size=s; index=0;
			}
								 ///<Increment
			CircularIndex & operator++() {
                ...
                return *this;
								 ...
								 ///<Increment 'i' positions
			CircularIndex & operator+=(int i) {
				index += i; while (index >= size) index -= size;
                return *this;
			}
								 ///<Increment 'i' positions
			CircularIndex & operator+(int i) {
				return CircularIndex(*this) += i;
			}
								 ///<Decrement
			CircularIndex & operator--() {
                ...
                return *this;
								 ...
								 ///<Decrement 'i' positions
			CircularIndex & operator-=(int i) {
				index -= i; while (index < 0) index += size;
                return *this;
			}
								 ///<Decrement 'i' positions
			CircularIndex & operator-(int i) {
			...
			operator int() {	 ///!Casting from CircularIndex to int, returning int.
				return index;
			}
								 ///!Assignment operator.
			CircularIndex & operator= (const CircularIndex &ci) {
				index = ci.index; size = ci.size;
                return *this;
			}
	};
	...
	class DigitalFilter
	{
		...
		public:
			...
			DigitalFilter() {	 ///<create void filter H(z) = 1
				a.push_back(1.0); b.push_back(1.0); gain = 1.0F;  prepare_state();
			}
								 ///<Change denominator (state conditions only cleaned if order changes)
			void set_a(std::vector<float> const &A) {
				a = A; prepare_state();
			}
								 ///<Change denominator (state conditions only cleaned if order changes)
			void set_b(const std::vector<float> &B) {
				b = B; prepare_state();
			}
								 ///<Create filter
			DigitalFilter(const DigitalFilter &f): a(f.a), b(f.b), gain(f.gain) {
				prepare_state();
			}
			///Asign operator (copy a filter from other)
			DigitalFilter & operator=(const DigitalFilter &f) {
				a = f.a; b = f.b; gain =f.gain; prepare_state();
                return *this;
			}
			...
			void clear() {		 ///<clean state conditions
				u.clear();
			}
								 ///<Filter one sample
			...
			void operator()(std::vector<float>::const_iterator begSrc,
				std::vector<float>::const_iterator endSrc,
								 ///<Filter several samples, from begSrc to endSrc, and save at begDst and following positions
